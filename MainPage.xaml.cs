...
private async Task<bool> EnsurePermissionsAndLocation()
{
                ...
                if (data.Contains("Led"))
                {
                    ...
                    foreach (string key in dict.Keys)
                    {
                        ...
                        if (key.Contains("Led"))
                        {
                            ...
                            {
                                case "Led1":
                                    SwitchLed(Led1, lightLevel, threshold, isSensorAlarm);
                                    break;
                                case "Led2":
                                    SwitchLed(Led2, lightLevel, threshold, isSensorAlarm);
                                    break;
                                case "Led3":
                                    SwitchLed(Led3, lightLevel, threshold, isSensorAlarm);
                                    break;
                                case "Led4":
                                    SwitchLed(Led4, lightLevel, threshold, isSensorAlarm);
                                    break;
                                case "Led5":
                                    SwitchLed(Led5, lightLevel, threshold, isSensorAlarm);
                                    break;
                                case "Led6":
                                    SwitchLed(Led6, lightLevel, threshold, isSensorAlarm);
                                    break;
                                default:
                                    break;
                            }
                    ...
                    if (dict["IsAlarm"][0] == "true") UpdateAlarmStatus(true);
    ...
    private void SwitchLed(Border led, string lightlevel, string threshold, string isSensorAlarm)
    {
        MainThread.BeginInvokeOnMainThread(async () =>
        {
            if (isSensorAlarm == "true") SetLedColorWithGradient(led, (Color)Resources["ColorLedRed"]);
            if (isSensorAlarm == "false") SetLedColorWithGradient(led, (Color)Resources["ColorLedBlue"]);
    ...
    private async Task SyncWithServer()
    {
        ...
        thresholdValue *= _thresholdMultiplier; 
